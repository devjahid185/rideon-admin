                ...
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        <form method="POST" action="{{ route($storeRoute) }}" enctype="multipart/form-data">
                                ...
                                <select class="form-control select2" name="service_types[]" multiple required>
                                    @foreach ($serviceTypes as $serviceType)
                                        <option value="{{ $serviceType->id }}"
                                            {{ in_array($serviceType->id, old('service_types', [])) ? 'selected' : '' }}>
                                            {{ $serviceType->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div id="max-weight-wrapper"
                                class="form-group {{ $errors->has('max_weight') ? 'has-error' : '' }}"
                                style="display:none;">
                                <label class="required">Maximum Weight to Carry (KG)</label>
                                <input class="form-control" type="number" step="0.01" name="max_weight"
